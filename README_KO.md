...
- **TUI** — 서버는 btop 스타일 실시간 대시보드(브라유 스파크라인 처리량, 연결별 진행률·파일별 속도/남은시간, 로그). 클라이언트는 진행률 바·실시간 속도·전체/파일별 남은 시간·전송 로그. 파이프에서는 한 줄 폴백.
- **MC 브라우저** — 경로 없이 실행하면 미드나잇 커맨더 스타일 픽커. space 선택, `/` 정규식 필터(매치 하이라이트), 목록 스크롤(PgUp/PgDn/g/G), 색상 구분(디렉터리/심볼릭/실행파일). pull은 서버 디렉터리를 원격 탐색.
